...
impl libp2p::swarm::NetworkBehaviour for FractalBehaviour {
    ...
    fn handle_established_outbound_connection(
        ...
        let gossipsub_handler = self.gossipsub.handle_established_outbound_connection(connection_id, peer, addr, role_override, port_use)?;
        let _kademlia_handler = self.kademlia.handle_established_outbound_connection(connection_id, peer, addr, role_override, port_use)?;
        let _request_response_handler = self.request_response.handle_established_outbound_connection(connection_id, peer, addr, role_override, port_use)?;
        let _connection_limits_handler = self.connection_limits.handle_established_outbound_connection(connection_id, peer, addr, role_override, port_use)?;
        let _allow_block_list_handler = self.allow_block_list.handle_established_outbound_connection(connection_id, peer, addr, role_override, port_use)?;
...
impl FractalBehaviour {
    pub fn new(peer_id: PeerId) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let mut kademlia_config = libp2p::kad::Config::new(protocol);
